import{s as p,j as a}from"./index-091168b2.js";import{g as l}from"./config-c632e6c9.js";const u=document.querySelector("iframe"),e=await p({iframe:u,remoteUrl:l().toString(),blueprint:{preferredVersions:{wp:"latest",php:"8.2"},features:{networking:!0},landingPage:"/"}}),d=await fetch("./blueprints.phar"),c=new Uint8Array(await d.arrayBuffer());await e.writeFile(a(await e.documentRoot,"blueprints.phar"),c);const r=document.getElementById("output");try{const n=await(await fetch("https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar")).arrayBuffer();await e.writeFile("/wordpress/wp-cli.phar",new Uint8Array(n)),r.textContent+=`Running the Blueprint...
...
";
		...
		`});r.textContent+=o.text,console.log(o.text)}catch(s){throw console.error(s),r.textContent=s+"",s}console.log(await e.listFiles("/wordpress/wp-content/plugins"));
//# sourceMappingURL=php-blueprints-8f44eb4d.js.map
